...
import { GetCustomerQueries, NotificationType } from "../../../lib/interfaces";
...
export default function CustomersTab(props: Props) {
    const { pageInfo } = props;
    const { getCustomers, fetchedData, fetchedMsg, isLoading } = useCustomers();
    const [filter, setFilter] = useState<GetCustomerQueries>({ page: 1, limit: 10 })

    const pathLabels: Array<null | string> = [];
    pageInfo.keyPath.forEach(item => pathLabels.push(mapPageLabel(menuItems, item)));
    pathLabels.reverse();

    const headers = ["name", "email", "role", "isActive", "isDeleted", "rating", "updatedAt", "totalPaid", "totalOrders"];
        ...
        <>
            ...
            <div className="filter-group flex justify-between flex-wrap">
                <div className="left flex flex-wrap items-center mt-[24px]">
                    ...
                    <div className="btn-group">
                        <DarkButton content="Country" iconR="arrow_drop_down" onClick={() => { }} otherStyles="rounded-l-lg" />
                        <DarkButton content="Source" iconR="arrow_drop_down" onClick={() => { }} />
                        <DarkButton content="More Filters" iconR="arrow_drop_down" onClick={() => { }} otherStyles="rounded-r-lg" />
                    </div>
                </div>
            ...
            <div className="bg-[#070c29] p-[16px] rounded-lg mt-[24px]">
                <div style={{ border: "0", padding: "0" }} className="table-container">
                    <Table isLoading={isLoading} data={fetchedData && fetchedData.users && fetchedData.users} headers={headers} handleSetFilterSort={handleSetFilterSort} hasActions={true}/>
                </div>
            </div>
